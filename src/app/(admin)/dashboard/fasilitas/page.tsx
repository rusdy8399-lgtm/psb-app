...
import { Plus, Edit, Trash2, Building2, Upload, ImageIcon } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function FasilitasCMSPage() {
  const [data, setData] = useState<FasilitasData[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<FasilitasData | null>(null);
  ...
  const handleOpenDialog = (item: FasilitasData | null = null) => {
    if (item) {
      setEditingItem(item);
      setFormData({
        nama: item.nama,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const url = editingItem ? `/api/admin/fasilitas/${editingItem.id}` : "/api/admin/fasilitas";
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-8 bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Kelola daftar sarana dan prasarana penunjang kenyamanan santri.</p>
        </div>
            ...
            <Table>
              <TableHeader className="bg-slate-50/50 border-b border-slate-100 h-14">
                <TableRow className="hover:bg-transparent border-none">
                  ...
                  <TableHead className="font-bold text-[10px] uppercase tracking-widest text-slate-400">Nama Fasilitas</TableHead>
                ...
                {data.length > 0 ? (
                  data.map((item) => (
                    <TableRow key={item.id} className="hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-none group">
                      ...
                      <TableCell>
                        <p className="font-bold text-slate-700 text-sm leading-snug">{item.nama}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1.5 opacity-60 line-clamp-1 max-w-md italic">{item.deskripsi}</p>
                      </TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={3} className="h-40 text-center text-slate-400 font-medium italic">Belum ada data fasilitas.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
        ...
        <DialogContent className="max-w-2xl overflow-y-auto max-h-[90vh] rounded-[24px] border-none shadow-2xl p-0">
          ...
          <form onSubmit={handleSave} className="p-8 space-y-8">
            <div className="space-y-2.5">
              <Label className="font-bold text-slate-700 text-sm ml-1">Nama Fasilitas</Label>
              <Input 
                required 
                value={formData.nama} 
                onChange={(e) => setFormData({...formData, nama: e.target.value})} 
                placeholder="Contoh: Gedung Madrasah Unit 2" 
                className="h-12 rounded-xl border-slate-200 bg-slate-50/30 focus:bg-white shadow-none focus:ring-2 focus:ring-primary/10"
              />
            </div>
            <div className="space-y-2.5">
              <Label className="font-bold text-slate-700 text-sm ml-1">Deskripsi Fasilitas</Label>
              <Textarea 
                required 
                value={formData.deskripsi} 
                onChange={(e) => setFormData({...formData, deskripsi: e.target.value})} 
                placeholder="Tuliskan kegunaan atau kapasitas fasilitas ini..."
                className="min-h-[120px] rounded-xl border-slate-200 bg-slate-50/30 focus:bg-white shadow-none focus:ring-2 focus:ring-primary/10"
              />
            </div>
            ...
            <div className="space-y-4 p-6 bg-[#FAFAF8] rounded-2xl border border-slate-100">
               <Label className="font-bold text-slate-700 text-sm flex items-center gap-2 mb-2">
                 <div className="w-1.5 h-1.5 rounded-full bg-primary" /> Foto Fasilitas
               </Label>
               <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                  ...
                  <div className="flex-1 space-y-3">
                     ...
                     <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest opacity-60">Klik area preview untuk mengunggah penampakan fasilitas.</p>
                  </div>
               </div>
            </div>
            ...
            <DialogFooter className="pt-8 border-t border-slate-100 flex gap-3">
              ...
              <Button type="submit" className="h-12 bg-primary hover:bg-[#143d24] font-bold rounded-xl px-10 shadow-lg shadow-primary/20 transition-all active:scale-[0.98]" disabled={isUploading}>
                {isUploading ? "Mengunggah..." : (editingItem ? "Simpan Perbaikan" : "Publish Fasilitas")}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
